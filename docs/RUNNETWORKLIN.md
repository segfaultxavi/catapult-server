...
2. Announce a valid [VotingKeyLinkTransaction](https://docs.symbolplatform.com/serialization/coresystem.html#votingkeylinktransaction).
   ...
   * Replace ``<VOTING_PUBLIC_KEY>`` with the public key obtained from ``catapult.tools.votingkey``.
    ...
    > **NOTE:**
    > Do not backup the ``../votingkeys/private_key_tree1.dat`` file (or any file in the ``votingkeys`` directory). If they are lost or compromised, just unlink the previous voting key and link a new one repeating the above procedure.
